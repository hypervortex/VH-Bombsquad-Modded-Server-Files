...
"""
    ...
    description: A version of race where you must only move when the light is
    green. This mod originated with the squid race minigame (I believe this
    was created by byANG3L and Chocolarito), but I wanted to make it less creepy
    (more kid-friendly). In the process, I had to modify quite a bit.
    author: levyfellow
    tested on: 1.7
"""
...
# ba_meta export game
class RLGLGame(ba.TeamGameActivity[Player, Team]):
    """Game of Red Light Green Light"""

    name = 'Red Light/Green Light'
    description = 'Stop when the light is red!'
    ...
    def get_available_settings(
        ...
        settings = [
            ba.IntSetting('Laps', min_value=1, default=1, increment=1),
    ...
    def __init__(self, settings: dict):
        ...
        self._rlgl_lights: Optional[List[ba.Node]] = None
        ...
        self._sensitivity = float(settings['Sensitivity'])
        self._penalty = int(settings['Movement Penalty'])
        ...
        for i in range(2):
            ...
            ba.animate(
                lnub, 'opacity', {
                    4.0 * t_scale: 0,
                    5.0 * t_scale: 1.0})
            self._rlgl_lights.append(lnub)
        self._rlgl_lights[0].color = (0.2, 0, 0)
        self._rlgl_lights[1].color = (0.0, 0.3, 0)
    ...
    def _green_light(self) -> None:
        delay = random.uniform(3,8)
        #bs.screenmessage(str(delay), color=(0,1,0))

        # update the lights
        self._rlgl_lights[0].color = (0.2, 0, 0)
        self._rlgl_lights[1].color = (0.0, 1.0, 0)

        # set the flags
        self._do_delete = False
        self._ticks = True

        ba.timer(delay, self._red_light)

    def _red_light(self) -> None:
        # update the lights
        self._rlgl_lights[0].color = (1.0, 0, 0)
        self._rlgl_lights[1].color = (0.0, 0.3, 0)

        # set the flags
        self._ticks = False

        # give some time before watching
        ba.timer(self._sensitivity, self._check_delete)
    ...
    def _check_delete(self) -> None:
        ...
        ba.timer(delay, self._green_light)

    def _start_delete(self) -> None:
        for player in self.players:
            if player.is_alive() and self._do_delete:
                # see if the player is moving
                if (player.actor.node.move_up_down or
                    player.actor.node.move_left_right):

                    player.delay = self._start_time - time.time() + self._penalty
                    player.actor.handlemessage(ba.DieMessage())
                    # NewBlast(
    ...
    def _start_race(self) -> None:
        ...
        self._green_light()
        self._do_ticks()
        ba.timer(0.1, self._start_delete, repeat=True)
    ...
    def handlemessage(self, msg: Any) -> Any:
        ...
        if isinstance(msg, ba.PlayerDiedMessage):
            ...
            if not player.finished:
                self.respawn_player(player, respawn_time = player.delay)
                player.delay = 1
