...
def log(msg: str, mtype: str = "sys") -> None:
    ...
    if SETTINGS["discordbot"]["enable"]:
        message = msg.replace("||", "|")
        if mtype == 'chat':
            discord_bot.push_log("<:ChatBot:912764845072461834> " + message)
        elif mtype == 'playerjoin':
            discord_bot.push_log("<:botAdded:880340694055661599> " + message)
        elif mtype == "chatcmd":
            discord_bot.push_log("<:commands:880508766217453598> " + message)
        elif mtype == "ban":
            discord_bot.push_log("<:BAN:1120582788798758982> " + message)
        else:
            discord_bot.push_log("<:Warning_:912790035080806420> " + message)
